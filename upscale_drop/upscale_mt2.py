...
def calc_tile_position(width, height, col, row):
    # generate mask
    tile_left = col * tile_size[0]
    tile_top = row * tile_size[1]
    tile_right = (col + 1) * tile_size[0] + padding
    tile_bottom = (row + 1) * tile_size[1] + padding
    if tile_right > height:
        tile_right = height
        tile_left = height - tile_size[0] - padding * 1
    if tile_bottom > width:
        tile_bottom = width
        tile_top = width - tile_size[1] - padding * 1

    return tile_top, tile_left, tile_bottom, tile_right

def thread_infer(engine, image, row, col):
    ...
    target_tile_size = (int((tile_size[0] + padding * 1) * upscale_ratio),
                        int((tile_size[1] + padding * 1) * upscale_ratio))

    tile_left, tile_top, tile_right, tile_bottom = calc_tile_position(width, height, row, col)
    # 裁剪瓦片
...
class UpscaleModel:
    ...
    def extract_and_enhance_tiles(self, image, upscale_ratio=2.0):
        ...
        image_list = [image] * (num_rows*num_cols)
        row_list = []
        col_list = []
        for row in range(num_rows):
            for col in range(num_cols):
                row_list.append(row)
                col_list.append(col)
        thread_args = zip(engine_list, image_list, row_list, col_list)

        with ThreadPoolExecutor(max_workers=8) as pool:
            results = pool.map(thread_infer, *thread_args)
        img_tiles = []
        img_h_tiles = []
        ...
        # 二维的 2*3=6
        # print(f"==========={len(img_tiles)}, {len(img_h_tiles)} = {len(img_tiles)*len(img_h_tiles)}===========")
