...
def validate(i, val_dataloader, model, criterion, val_loss_meter,
             val_psnr_meter, writer, config, device):
...
def setup_optimizer(model, config):
    ...
    if config['training']['optimizer']['name'] == 'sgd':
        optimizer = optim.SGD(model.parameters(), lr=base_lr, momentum=config['training']['optimizer']['momentum'])
    else:
...
def main(args_cuda, args_config):
    # Load config file.
    with open(args_config, 'r') as f:
        config = yaml.load(f, Loader=yaml.FullLoader)

#    if args_cuda and not torch.cuda.is_available():
#        raise Exception('No GPU found')
    ...
    sys.stdout.flush()
    model = ESPCNSeq(img_channels=config['model']['img_channels'],
        ...
        for batch in train_dataloader:
            ...
            if i % config['training']['val_interval'] == 0:
                ...
                if val_psnr_meter.avg >= best_val_psnr:
                    ...
                    path = '{}/{}/{}_{}_reduced-channels.pt'.format(
                        config['training']['checkpoint_folder'],
                        os.path.basename(args_config)[:-5],
                        os.path.basename(args_config)[:-5], i)
                    path_jit = '{}/{}/{}_{}_jit.pt'.format(
                        config['training']['checkpoint_folder'],
                        os.path.basename(args_config)[:-5],
                        os.path.basename(args_config)[:-5], i)
                    ...
                    # traced_script_module.save(path_jit)

#                    traced_script_module.save("./models/espcnseq/espcnseq_test.pt")
...
if __name__ == '__main__':
    ...
    main(True, "./configs/espcnseq.yaml")
    '''
